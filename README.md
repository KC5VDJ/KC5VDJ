...
20240412 oh, and this version will try to be devoid of stdio for anything but host-side messaging....AAAAARRRRRRGGGGHHHHH!!!  How long to itemize three tapes?  disk as well...  in 2024, the sweet spot is closer to 4 megs to 16 mega per block transferred.  all but the tightest pi zero original, and EVEN THEY TOO (maybe), should be able to do at least a single buffer for a single host block at max efficiency block size, say in types.....  the biggest io bottleneck in dtcyber currently is the very portable, but usually pawned off to the os directly, stdio package.  the solution is obviously lower level file io and diy buffering.
